...
const DIST_FILE = path.join(SRC_DIR, 'index.html');
...
function assetVersion(file) {
  try {
    return Math.floor(fs.statSync(path.join(SRC_DIR, file)).mtimeMs);
  } catch (err) {
    return Date.now();
  }
}
...
function compile() {
  ...
  try {
    ...
    html = html
      .replace('src="js/theme.js"', `src="js/theme.js?v=${assetVersion('js/theme.js')}"`)
      .replace('href="css/styles.css"', `href="css/styles.css?v=${assetVersion('css/styles.css')}"`);
    ...
    JS_FILES.forEach(file => {
      html += `\n<script src="${file}?v=${assetVersion(file)}"></script>`;
    });
