...
function exportAnx6PDF() {
  ...
  const drawHeaderFooter = (data) => {
    ...
    doc.text('Enforcement & Monitoring Guidelines for Sand Mining', pageWidth - 40, 28, { align: 'right' });

    const bottomY = pageHeight - 45;
    doc.setLineWidth(0.5);
    doc.setDrawColor(0, 0, 0);
    doc.line(55, bottomY, pageWidth - 55, bottomY);
    doc.setFont('times', 'bold');
    doc.setFontSize(7);
    doc.setTextColor(0, 0, 0);
    doc.text(`PREPARED BY: SUB-DIVISIONAL COMMITTEE OF ${districtUpper} DISTRICT`, pageWidth / 2, bottomY + 13, { align: 'center' });
    doc.text('ASSISTED BY: RSP GREEN DEVELOPMENT AND LABORATORIES PVT. LTD', pageWidth / 2, bottomY + 22, { align: 'center' });
    doc.text(String(393 + data.pageNumber), pageWidth - 35, pageHeight - 18, { align: 'right' });
  };
...
function renderPdfUploadUIAnx6() {
  ...
  if (els.sec && els.sec.style.display === 'block' && els.iframe) {
    const url = '/api/download-pdf?projectId=' + S.activeProject.id + '&annexureId=anx6&inline=true';
...
function togglePDFPreviewAnx6() {
  ...
  if (sec.style.display === 'block') {
    ...
    iframe.src = '/api/download-pdf?projectId=' + S.activeProject.id + '&annexureId=anx6&inline=true';
...
function downloadPdfAnx6() {
  ...
  a.href = '/api/download-pdf?projectId=' + S.activeProject.id + '&annexureId=anx6';
