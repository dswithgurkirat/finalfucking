...
function normalizeHeaderAnx7(value) {
...
function findHeaderIndexAnx7(rows, expectedHeaders) {
  return rows.findIndex(row => {
    const normalized = row.map(normalizeHeaderAnx7);
    const required = expectedHeaders.slice(0, Math.min(5, expectedHeaders.length)).map(normalizeHeaderAnx7);
    return required.every(header => normalized.some(cell => cell === header || cell.includes(header) || header.includes(cell)));
  });
}

function splitLogicalGroupsAnx7(rows, expectedHeaders) {
  const cleanRows = rows.filter(row => Array.isArray(row));
  const headerIndex = findHeaderIndexAnx7(cleanRows, expectedHeaders);
  if (headerIndex < 0) throw new Error('Missing columns. Please use the Annexure-7 template columns.');
  const groups = [];
  let current = [];
  for (let i = headerIndex + 1; i < cleanRows.length; i += 1) {
    const row = cleanRows[i];
    const nonEmpty = row.some(cell => String(cell ?? '').trim() !== '');
    const looksLikeHeader = findHeaderIndexAnx7([row], expectedHeaders) === 0;
    if (!nonEmpty || looksLikeHeader) {
      if (current.length) {
        groups.push(current);
        current = [];
      }
  ...
  if (!groups.length) throw new Error('No data rows found after the header.');
  return groups;
}
  ...
  try {
    parseWorkbookRowsAnx7(file, sheets => {
      ...
      sourceSheets.forEach(sheet => {
        const groups = splitLogicalGroupsAnx7(sheet.rows, ANX7_ROUTE_HEADERS);
        groups.forEach((group, index) => {
          ...
          fillRouteCardAnx7(card, group, file.name);
          loaded += group.length;
        });
      });
      renumberTableBlocksAnx7('individual');
...
function uploadClustersAnx7(event, input) {
  ...
  try {
    parseWorkbookRowsAnx7(file, sheets => {
      ...
      let loaded = 0;
      sourceSheets.forEach(sheet => {
        const groups = splitLogicalGroupsAnx7(sheet.rows, ANX7_CLUSTER_HEADERS);
        groups.forEach(group => {
          ...
          fillClusterCardAnx7(card, group, file.name);
          loaded += group.length;
        });
      });
      renumberTableBlocksAnx7('cluster');
...
function drawAnx7Furniture(doc, pageNum) {
  ...
  doc.setDrawColor(0, 0, 0);
  doc.line(margin + 15, footerY, pageW - margin - 15, footerY);
  doc.setFont('times', 'bold');
  doc.setFontSize(8);
  doc.setTextColor(0, 0, 0);
  doc.text(`PREPARED BY: SUB-DIVISIONAL COMMITTEE OF ${district.toUpperCase()} DISTRICT`, pageW / 2, footerY + 5, { align: 'center' });
  doc.text('ASSISTED BY: RSP GREEN DEVELOPMENT AND LABORATORIES PVT. LTD', pageW / 2, footerY + 9, { align: 'center' });
  doc.setFontSize(10);
  doc.text(String(400 + pageNum), pageW - margin - 2, pageH - margin - 2, { align: 'right' });
}

function exportAnx7PDF() {
...
function renderPdfUploadUIAnx7() {
  ...
  if (els.sec && els.sec.style.display === 'block' && els.iframe) {
    const url = '/api/download-pdf?projectId=' + S.activeProject.id + '&annexureId=anx7&inline=true';
    if (!els.iframe.src.includes('anx7')) els.iframe.src = url;
  }
  if (window.initLucide) window.initLucide();
}
...
function togglePDFPreviewAnx7() {
  ...
  if (sec.style.display === 'block') {
    closePDFPreviewAnx7();
  } else if (S.activeProject?.anx7PdfName) {
    iframe.src = '/api/download-pdf?projectId=' + S.activeProject.id + '&annexureId=anx7&inline=true';
...
function downloadPdfAnx7() {
  ...
  a.href = '/api/download-pdf?projectId=' + S.activeProject.id + '&annexureId=anx7';
...
async function deletePdfAnx7() {
  ...
  try {
    const res = await fetch('/api/upload-pdf', {
      ...
      body: JSON.stringify({ projectId: S.activeProject.id, fileName: null, pdf: null, annexureId: 'anx7' })
    });
    const data = await res.json();
    if (data.success) {
      S.activeProject.anx7PdfName = null;
      const index = S.projects.findIndex(p => p.id === S.activeProject.id);
      if (index >= 0) S.projects[index].anx7PdfName = null;
  ...
  reader.onload = async function(e) {
    ...
    try {
      const res = await fetch('/api/upload-pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ projectId: S.activeProject.id, fileName: file.name, pdf: b64, annexureId: 'anx7' })
      });
      const data = await res.json();
      if (data.success) {
        S.activeProject.anx7PdfName = file.name;
        const index = S.projects.findIndex(p => p.id === S.activeProject.id);
        if (index >= 0) S.projects[index].anx7PdfName = file.name;
        const iframe = document.getElementById('pdf-iframe-anx7');
        const sec = document.getElementById('pdf-preview-section-anx7');
